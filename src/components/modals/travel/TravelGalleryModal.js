...
export default function TravelGalleryModal({show, travelId, setMainPhoto, updateMainPhoto, onHide}) {
    ...
    const fetchImages = async () => {
        try {
            let response = await axios.get(`${DOMAIN_API_URL}/images/travel/${travelId}`);

            const updatedImages = await Promise.all(response.data.map(async (image) => {
                const blobResponse = await axios.get(`${DOMAIN_API_URL}/images/travel/${travelId}/name/${image}`, {responseType: 'arraybuffer'});
    ...
    const handleSetMainPhoto = async () => {
        ...
        await axios.put(`${DOMAIN_API_URL}/images/travel/${travelId}/name/${selectedImage.fileName}/main`,
            {}, config)
            .then(() => setMainPhoto(travelId, selectedImage.url))
    ...
    const handleDeletePhoto = async () => {
        ...
        await axios.delete(`${DOMAIN_API_URL}/images/travel/${travelId}/name/${selectedImage.fileName}`, config)
            .then(() => {
                updateMainPhoto(travelId);
    ...
    const handleUploadImageSubmit = async (event) => {
        ...
        await axios.post(`${DOMAIN_API_URL}/images/travel/${travelId}`, formData, config)
            .then(response => {
                ...
                <Modal.Body>
                    ...
                    {selectedImage &&
                        <ButtonGroup className="d-flex">
                            <Button className="col-6 travel-btn-primary" onClick={handleSetMainPhoto}>Set as main photo</Button>
