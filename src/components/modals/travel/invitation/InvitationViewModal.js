...
export default function InvitationViewModal({show, invite, showInviteStatus, fetchInvites, updateInvitations, onHide}) {

    const setInvitationAsViewed = async () => {
        ...
        await axios.put(`${DOMAIN_API_URL}/invitations/${invite.id}/viewed`, {}, config)
            .then(() => updateInvitations)
            .catch(error => console.error(error));
    };

    useEffect(() => {
        if (show && invite && !invite.viewed) {
            setInvitationAsViewed();
        }
    }, [invite])
    ...
    const handleApproveClick = async () => {
        ...
        await axios.post(`${DOMAIN_API_URL}/invitations/${invite.id}/approve`, {}, config)
            .then(() => fetchInvites())
            .catch(error => console.error(error));
    };
    ...
    const handleRejectClick = async () => {
        ...
        await axios.post(`${DOMAIN_API_URL}/invitations/${invite.id}/reject`, {}, config)
            .then(() => fetchInvites())
            .catch(error => console.error(error));
    };
            ...
            <Modal show={show} animation={false} onHide={onHide}>
                ...
                <Modal.Body>
                    <div className="d-flex justify-content-between mb-2">
                        <Form.Group className="d-flex align-items-center">
                            ...
                            <Link to="#" className="d-flex align-items-center"
                                ...
                                />
                                <p className="mb-0 me-2">{invite.travel.creator.name}</p>
                            </Link>
                        </Form.Group>
                        <Link to="#chat"
                        ...
                        <div className="d-flex border rounded-3 shadow-sm overflow-hidden mb-3 w-100">
                            ...
                            <div className="col-7 ps-3 p-2">
                                ...
                                <div className="travel-dates d-flex align-items-center py-2 mb-2">
                                    ...
                                    {moment(invite.travel.startDate).format("DD.MM.YYYY")}
                                    {' - '}
                                    {moment(invite.travel.endDate).format("DD.MM.YYYY")}
                                </div>
                                <div className="d-flex">
                                    {showTravelStatus(invite.travel)}
                                    <div className="d-flex justify-content-end align-items-end w-100">
                                        <Button as={Link} to={`/${invite.travel.id}`}
                    ...
                    {invite.status === "APPROVED" || invite.status === "REJECTED" ? (
                        <Button className="w-100 travel-btn-danger" onClick={handleDeleteClick}>Delete</Button>
                    ) : (
                        <ButtonGroup className="d-flex">
                            <Button className="col-6 travel-btn-success" onClick={handleApproveClick}>Approve</Button>
                            <Button className="col-6 travel-btn-danger" onClick={handleRejectClick}>Reject</Button>
                        </ButtonGroup>
                    )}
                </Modal.Body>
            </Modal>
